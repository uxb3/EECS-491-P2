...
public class CombatAgent extends edu.cwru.sepia.agent.Agent{
	...
	private void doAggro(StateView state) {
		for (Map.Entry<Integer, Action> order : unitOrders.entrySet()) {
			if (order.getValue() == null) //if it has no orders  
			{
				...
				double closetDis= Double.POSITIVE_INFINITY;
				int closetID=0;
				double distance=0;
...
//					}
					for (Integer enemyUnitID : state.getUnitIds(enemy)) {
						UnitView enemyUnit = state.getUnit(enemyUnitID);
						//get the chebyshev distance (which is the base distance for warcraft 2)
						distance=DistanceMetrics.chebyshevDistance(ux, uy, enemyUnit.getXPosition(), enemyUnit.getYPosition());
						 
						if (closetDis > distance ) {
							//(if you can see it)
							closetDis=distance;
							closetID=enemyUnitID;						
						}
						
					}
					
					unitOrders.put(order.getKey(), Action.createCompoundAttack(order.getKey(), closetID));
					foundsomething=true;
